...
这篇指南会指导你如何为 `wya-assist` 贡献一份自己的力量，请在你要提 `issue` 或者 `pull request` 之前花几分钟来阅读一遍这篇指南。
...
我们使用 [`GitHub Issues`](https://github.com/wya-team/wya-assist/issues) 来做 `bug` 追踪。

在你报告一个 `bug` 之前，请先确保已经搜索过已有的 `issue` 和阅读了我们的 [常见问题](https://github.com/wya-team/wya-assist/wiki/FAQ)。
...
在你 `clone` 了 `wya-assist`  的代码并且使用 `npm install` 安装完依赖后，你还可以运行下面几个常用的命令：

1. `npm start` 或者 `npm run dev` 在本地运行 `wya-assist`。
2. `npm run lint` / `npm run lint:fix` / `npm run lint:watch` 检查代码风格/修复/监听。
3. `npm run build:dev` / `npm run build` 构建 `wya-assist` 到 `dist` 目录。
